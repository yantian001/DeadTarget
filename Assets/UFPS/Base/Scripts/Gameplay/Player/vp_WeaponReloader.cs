...
public class vp_WeaponReloader : MonoBehaviour
{
    ...
    protected virtual bool CanStart_Reload()
    {
        ...
        {
            vp_ItemType item = vp_TargetEventReturn<vp_ItemType>.Send(this.transform, "GetItemType");
            if (item != null)
            {
                if (item is vp_UnitBankType)
                {
                    vp_UnitBankType bitem = item as vp_UnitBankType;
                    if (bitem != null && bitem.Reloadable)
                    {
                        if (m_Player.AddItem.Try(new object[] { bitem.Unit, bitem.Capacity }))
                        {
                            return true;
                        }
                    }
                }
                // m_Player.AddItem.Try();
            }
            return false;
        }
